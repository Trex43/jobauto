...
export async function queueAutoApply(opts: { userId: string; jobId: string; priority?: number; requireCoverLetter?: boolean }) {
  ...
  const existingQueue = await prisma.autoApplyJob.findFirst({ where: { userId, jobId, status: { in: ['QUEUED', 'IN_PROGRESS'] } } });
  if (existingQueue) return existingQueue;

  const applyMethod = detectApplyMethod(job.portal, job.applyUrl) as any;

  const autoApplyJob = await prisma.autoApplyJob.create({
    data: { userId, jobId, status: 'QUEUED', priority, applyMethod, scheduledAt: new Date() },
  });
...
export async function bulkQueueAutoApply(userId: string) {
  ...
  const appliedJobIds = applications.map((a: any) => a.jobId);
...
export async function cancelAutoApply(autoApplyJobId: string, userId: string) {
  ...
  if (job.status === 'IN_PROGRESS') throw new Error('Cannot cancel a job in progress');
  await prisma.autoApplyJob.update({ where: { id: autoApplyJobId }, data: { status: 'SKIPPED', errorMessage: 'Cancelled by user' } });
}
...
export async function getAutoApplyStats(userId: string) {
  const [total, byStatus, settings] = await Promise.all([
    ...
    prisma.autoApplyJob.groupBy({ by: ['status'], where: { userId }, _count: { status: true } }),
    prisma.userAutoApplySettings.findUnique({ where: { userId } }),
  ]);
  const stats: Record<string, number> = {};
  for (const row of byStatus) stats[row.status] = row._count.status;
